...
class AttackL0:
    def __init__(self, sess, model, bus,
    ...
    def gradient_descent(self, sess, model, bus):
        def compare(x, y):
            if self.TARGETED:
                return x == y
            else:
        ...
        tmeasurement = tf.Variable(np.zeros(shape, dtype=np.float32))
        tstate = tf.Variable(np.zeros((1, model.output_size), dtype=np.float32))
        const = tf.placeholder(tf.float32, [])

        # and the assignment to set the variables
        assign_delta = tf.placeholder(np.float32, shape)
        ...
        assign_tmeasurement = tf.placeholder(np.float32, shape)
        assign_tstate = tf.placeholder(np.float32, (1, self.model.output_size))

        # these are the variables to initialize when we run
        set_modifier = tf.assign(delta, assign_delta)
        setup = []
        setup.append(tf.assign(canchange, assign_canchange))
        setup.append(tf.assign(tmeasurement, assign_tmeasurement))
        setup.append(tf.assign(original, assign_original))
        setup.append(tf.assign(simg, assign_simg))
        setup.append(tf.assign(tstate, assign_tstate))

        new_measurement = (tf.tanh(delta + simg) / 2) * canchange + (1 - canchange) * original

        new_estimated_state = model.predict(new_measurement)
        estimated_new_measurement = bus.estimated(new_estimated_state)


        # sum up the losses
        # loss2:
        loss2 = tf.reduce_sum(tf.square(new_measurement - tf.tanh(tmeasurement) / 2))
        loss1 = tf.reduce_sum()
        loss = self.const * loss1 + loss2

        outgrad = tf.gradients(loss, [delta])[0]
        ...
        init = tf.variables_initializer(var_list=[delta, canchange, simg,
                                                  original, tmeasurement, tstate] + new_vars)
